...
class WeiXin
{
    ...
    public function responseNewsMessage($items)
    {
        ...
        if ($items && is_array($items)) {
            foreach ($items as $item) {
                if (is_array(
                        $item
                    ) && (isset($item['Title']) || isset($item['Description']) || isset($item['PicUrl']) || isset($item['Url']))) {
    ...
    public function sendTextMessage($openid, $content)
    {
        if ($this->getAccessToken()) {
            ...
            return self::post(
                self::$links['message']."?access_token={$this->access_token}",
    ...
    public function groupCreate($name)
    {
        if ($this->getAccessToken()) {
            ...
            return self::post(
                self::$links['group_create']."?access_token={$this->access_token}",
                json_encode($message, JSON_UNESCAPED_UNICODE)
            );
        }
    ...
    public function groupGet($openid)
    {
        if ($this->getAccessToken()) {
            ...
            return self::post(
                self::$links['group_getid']."?access_token={$this->access_token}",
                json_encode($message, JSON_UNESCAPED_UNICODE)
            );
        }
    ...
    public function groupRename($groupid, $name)
    {
        if ($this->getAccessToken()) {
            ...
            return self::post(
                self::$links['group_rename']."?access_token={$this->access_token}",
                json_encode($message, JSON_UNESCAPED_UNICODE)
            );
        }
    ...
    public function groupMove($openid, $to_groupid)
    {
        if ($this->getAccessToken()) {
            ...
            return self::post(
                self::$links['group_move']."?access_token={$this->access_token}",
                json_encode($message, JSON_UNESCAPED_UNICODE)
            );
        }
    ...
    public function userGet($next_openid = null)
    {
        if ($this->getAccessToken()) {
            if ($next_openid) {
                ...
                return self::get(
                    self::$links['user_get']."?access_token={$this->access_token}&next_openid={$next_openid}"
                );
            }
        }
    ...
    public function menuCreate($menus)
    {
        if ($this->getAccessToken()) {
            ...
            return self::post(
                self::$links['menu_create']."?access_token={$this->access_token}",
                json_encode($message, JSON_UNESCAPED_UNICODE)
            );
        }
    ...
    public function qrcodeCreate($sceneid, $expire_seconds = 0)
    {
        if ($this->getAccessToken()) {
            ...
            return self::post(
                self::$links['qrcode']."?access_token={$this->access_token}",
                json_encode($message, JSON_UNESCAPED_UNICODE)
            );
        }
    ...
    {
        return self::get(
            self::$links['oauth_access_token']."?appid={$this->appid}&secret={$this->appsecret}&code={$code}&grant_type=authorization_code"
        );
    }
    ...
    {
        return self::get(
            self::$links['oauth_refresh']."?appid={$this->appid}&grant_type=refresh_token&refresh_token={$refresh_token}"
        );
    }
