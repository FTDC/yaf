...
class Crypt_Blowfish
{
    ...
    function encrypt($plainText)
    {
        ...
        for ($i = 0; $i < $len; $i += 8) {
            list(, $Xl, $Xr) = ($this->_unpackMode == PS_UNPACK_NATIVE ? unpack(
                'N2',
                substr($plainText, $i, 8)
            ) : $this->myUnpackN2(substr($plainText, $i, 8)));
    ...
    function decrypt($cipherText)
    {
        ...
        for ($i = 0; $i < $len; $i += 8) {
            list(, $Xl, $Xr) = ($this->_unpackMode == PS_UNPACK_NATIVE ? unpack(
                'N2',
                substr($cipherText, $i, 8)
            ) : $this->myUnpackN2(substr($cipherText, $i, 8)));
    ...
    function myUnpackN($str)
    {
        if (pack('L', 0x6162797A) == pack('V', 0x6162797A)) {
            return ((ord($str) << 24) | (ord(substr($str, 1)) << 16) | (ord(substr($str, 2)) << 8) | ord(
                    substr($str, 3)
                ));
        } else {
            return (ord($str) | (ord(substr($str, 1)) << 8) | (ord(substr($str, 2)) << 16) | (ord(
                        substr($str, 3)
                    ) << 24));
        }
    }
