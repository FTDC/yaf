...
abstract class Builder
{
    ...
    protected function parseData(Query $query, $data = [], $fields = [], $bind = [], $suffix = '')
    {
        ...
        foreach ($data as $key => $val) {
            ...
            if ($val instanceof Expression) {
                ...
                continue;
            } elseif (!is_scalar($val) && (in_array(
                        $key,
                        (array)$query->getOptions('json')
            ...
            if (false !== strpos($key, '->')) {
                ...
                $item = $this->parseKey($query, $key);
                $result[$item] = 'json_set('.$item.', \'$.'.$name.'\', '.$this->parseDataBind(
                        $query,
                        $key,
                        $val,
                        $bind,
    ...
    protected function parseWhereItem(Query $query, $field, $val, $rule = '', $binds = [], $bindName = null)
    {
        ...
        if (is_scalar($value) && !in_array(
                $exp,
                ['EXP', 'NOT NULL', 'NULL', 'IN', 'NOT IN', 'BETWEEN', 'NOT BETWEEN']
    ...
    public function selectInsert(Query $query, $fields, $table)
    {
        ...
        return 'INSERT INTO '.$this->parseTable($query, $table, $options).' ('.implode(',', $fields).') '.$this->select(
                $options
            );
    }
