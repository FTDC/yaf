...
class Query
{
    ...
    protected function getJoinTable($join, &$alias = null)
    {
        ...
        if (is_array($join)) {
            ...
            if (false !== strpos($join, '(')) {
                ...
                if ($prefix && false === strpos($table, '.') && 0 !== strpos($table, $prefix) && 0 !== strpos(
                        $table,
                        '__'
                    )) {
    ...
    {
        if (is_array($field)) {
            $this->options['data'] = isset($this->options['data']) ? array_merge(
                $this->options['data'],
                $field
            ) : $field;
        } else {
    ...
    protected function parseArrayWhereItems($field, $logic)
    {
        ...
        if (!empty($where)) {
            $this->options['where'][$logic] = isset($this->options['where'][$logic]) ? array_merge(
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        /** @var Paginator $class */
        $class = false !== strpos($config['type'], '\\') ? $config['type'] : '\\think\\paginator\\driver\\'.ucwords(
                $config['type']
            );
    ...
    public function getPk($options = '')
    {
        if (!empty($this->pk)) {
            ...
            $pk = $this->connection->getPk(
                is_array($options) && isset($options['table']) ? $options['table'] : $this->getTable()
            );
        }
