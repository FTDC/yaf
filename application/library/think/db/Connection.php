...
abstract class Connection
{
    ...
    public static function instance($config = [], $name = false)
    {
        ...
        if (true === $name || !isset(self::$instance[$name])) {
            ...
            $class = false !== strpos($options['type'], '\\') ? $options['type'] : '\\think\\db\\connector\\'.ucwords(
                    $options['type']
                );
            // 记录初始化信息
    ...
    public function connect(array $config = [], $linkNum = 0, $autoConnection = false)
    {
        ...
        try {
            ...
            if ($config['debug']) {
                // 记录数据库连接信息
                $this->log(
                    '[ DB ] CONNECT:[ UseTime:'.number_format(microtime(true) - $startTime, 6).'s ] '.$config['dsn']
                );
            }
    ...
    public function find(Query $query)
    {
        ...
        if ($this->cache && !empty($options['cache']) && true === $options['cache']['key'] && is_string(
                $pk
            ) && isset($options['where']['AND'][$pk])) {
