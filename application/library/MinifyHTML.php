...
class MinifyHTML
{
    ...
    public static function minify($html, $options = array())
    {
        ...
        self::$_isXhtml = (isset($options['xhtml']) ? (bool)$options['xhtml'] : (false !== strpos(
                $html,
        ...
        $html = preg_replace(
            '/\\s+(<\\/?(?:area|base(?:font)?|blockquote|body'.'|caption|center|cite|col(?:group)?|dd|dir|div|dl|dt|fieldset|form'.'|frame(?:set)?|h[1-6]|head|hr|html|legend|li|link|map|menu|meta'.'|ol|opt(?:group|ion)|p|param|t(?:able|body|head|d|h||r|foot|itle)'.'|ul)\\b[^>]*>)/i',
            '$1',
            $html,
            Media::getBackTrackLimit()
        );
    ...
    protected static function _removeStyleCB($m)
    {
        ...
        return self::_reservePlace(
            self::_needsCdata($css) ? "{$openStyle}/*<![CDATA[*/{$css}/*]]>*/</style>" : "{$openStyle}{$css}</style>"
        );
    }
    ...
    protected static function _removeScriptCB($m)
    {
        ...
        return self::_reservePlace(
            self::_needsCdata($js) ? "{$openScript}/*<![CDATA[*/{$js}/*]]>*/</script>" : "{$openScript}{$js}</script>"
        );
    }
