...
function smarty_function_html_select_date($params, $template)
{
    ...
    if (isset($params['time']) && is_array($params['time'])) {
        if (isset($params['time'][$prefix.'Year'])) {
            ...
            foreach (array('Y' => 'Year', 'm' => 'Month', 'd' => 'Day') as $_elementKey => $_elementName) {
                ...
                $$_variableName = isset($params['time'][$prefix.$_elementName]) ? $params['time'][$prefix.$_elementName] : date(
                    $_elementKey
                );
            }
            ...
            foreach (array('Y' => 'Year', 'm' => 'Month', 'd' => 'Day') as $_elementKey => $_elementName) {
                $_variableName = '_'.strtolower($_elementName);
                $$_variableName = isset($params['time'][$field_array][$prefix.$_elementName]) ? $params['time'][$field_array][$prefix.$_elementName] : date(
                    $_elementKey
                );
            }
            $time = mktime(0, 0, 0, $_month, $_day, $_year);
    ...
    if ($display_months) {
        ...
        for ($i = 1; $i <= 12; $i++) {
            $_val = sprintf('%02d', $i);
            $_text = isset($month_names) ? smarty_function_escape_special_chars(
                $month_names[$i]
