...
abstract class Smarty_Internal_TemplateBase extends Smarty_Internal_Data
{
    ...
    public function fetch(
        ...
        $_template = ($template instanceof $this->template_class) ? $template : $this->smarty->createTemplate(
            $template,
            $cache_id,
            $compile_id,
            $parent,
        ...
        if (!($_template->caching == Smarty::CACHING_LIFETIME_CURRENT || $_template->caching == Smarty::CACHING_LIFETIME_SAVED) || !$_template->cached->valid) {
            ...
            if (!$_template->source->uncompiled) {
                ...
                if ($_template->source->recompiled) {
                    ...
                    try {
                        ob_start();
                        if (empty($_template->properties['unifunc']) || !is_callable(
                                $_template->properties['unifunc']
                            )) {
                            throw new SmartyException(
                    ...
                    switch (PHP_SAPI) {
                        case 'cli':
                            ...
                            !empty($_SERVER['SMARTY_PHPUNIT_DISABLE_HEADERS']) /* phpunit$ */
                                ...
                                $_SERVER['SMARTY_PHPUNIT_HEADERS'][] = 'Last-Modified: '.gmdate(
                                        'D, d M Y H:i:s',
