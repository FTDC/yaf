...
abstract class Smarty_Internal_TemplateCompilerBase
{
    ...
    public function compileTemplate(Smarty_Internal_Template $template)
    {
        ...
        if (!$this->suppressHeader) {
            $template_header .= "<?php /* Smarty version ".Smarty::SMARTY_VERSION.", created on ".strftime(
                    "%Y-%m-%d %H:%M:%S"
                )."\n";
    ...
    public function compileTag($tag, $args, $parameter = array())
    {
        ...
        if (in_array("'nocache'", $args) || in_array(array('nocache' => 'true'), $args) || in_array(
                array('nocache' => '"true"'),
                $args
            ...
            if (strlen($tag) < 6 || substr($tag, -5) != 'close') {
                ...
                if (isset($this->smarty->registered_objects[$tag]) && isset($parameter['object_methode'])) {
                    ...
                    if (!in_array(
                        ...
                        ) && (empty($this->smarty->registered_objects[$tag][1]) || in_array(
                                $methode,
                        ...
                        if ($plugin_type == Smarty::PLUGIN_COMPILER) {
                            ...
                            if (!is_array($function)) {
                                ...
                                if (is_object($function[0])) {
                                    return $this->smarty->registered_plugins[$plugin_type][$tag][0][0]->$function[1](
                                        $new_args,
                                        $this
                                    );
                                } else {
                ...
                foreach ($this->smarty->plugin_search_order as $plugin_type) {
                    if ($plugin_type == Smarty::PLUGIN_BLOCK && $this->smarty->loadPlugin(
                            'smarty_compiler_'.$tag
                        ) && (!isset($this->smarty->security_policy) || $this->smarty->security_policy->isTrustedTag(
                                $tag,
                                $this
                            ))) {
                        ...
                        if ($function = $this->getPlugin($tag, $plugin_type)) {
                            if (!isset($this->smarty->security_policy) || $this->smarty->security_policy->isTrustedTag(
                                    $tag,
                                    $this
                                )) {
                                return $this->callTagCompiler(
                    ...
                    if ($found) {
                        ...
                        if ($plugin_type == Smarty::PLUGIN_COMPILER) {
                            ...
                            if (!is_array($function)) {
                                ...
                                if (is_object($function[0])) {
                                    return $this->default_handler_plugins[$plugin_type][$tag][0][0]->$function[1](
                                        $new_args,
                                        $this
                                    );
                                } else {
                ...
                if (isset($this->smarty->registered_plugins[Smarty::PLUGIN_COMPILER][$tag])) {
                    ...
                    if (!is_array($function)) {
                        ...
                        if (is_object($function[0])) {
                            return $this->smarty->registered_plugins[Smarty::PLUGIN_COMPILER][$tag][0][0]->$function[1](
                                $args,
                                $this
                            );
                        } else {
    ...
    public function trigger_template_error($args = null, $line = null)
    {
        ...
        $match = preg_split("/\n/", $this->lex->data);
        $error_text = 'Syntax Error in template "'.$this->template->source->filepath.'"  on line '.$line.' "'.htmlspecialchars(
                trim(preg_replace('![\t\r\n]+!', ' ', $match[$line - 1]))
            ).'" ';
        if (isset($args)) {
