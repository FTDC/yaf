...
class PHPExcel_Shared_Date
{
    ...
    public static function ExcelToPHP($dateValue = 0, $adjustToTimezone = false, $timezone = null)
    {
        ...
        $timezoneAdjustment = ($adjustToTimezone) ? PHPExcel_Shared_TimeZone::getTimezoneAdjustment(
            $timezone,
            $returnValue
        ) : 0;
    ...
    public static function FormattedPHPToExcel($year, $month, $day, $hours = 0, $minutes = 0, $seconds = 0)
    {
        ...
        $excelDate = floor((146097 * $century) / 4) + floor((1461 * $decade) / 4) + floor(
                (153 * $month + 2) / 5
            ) + $day + 1721119 - $my_excelBaseDate + $excel1900isLeapYear;
        ...
        if (preg_match('/(^|\])[^\[]*['.self::$possibleDateFormatCharacters.']/i', $pFormatCode)) {
            ...
            if (strpos($pFormatCode, '"') !== false) {
                ...
                foreach (explode('"', $pFormatCode) as $subVal) {
                    ...
                    if (($segMatcher = !$segMatcher) && (preg_match(
                            '/(^|\])[^\[]*['.self::$possibleDateFormatCharacters.']/i',
                            $subVal
                        ))) {
                        return true;
                    }
