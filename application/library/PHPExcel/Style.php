...
class PHPExcel_Style extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
            ...
            if ($this->_isSupervisor) {
                ...
                if ($pAdvanced && isset($pStyles['borders'])) {
                    ...
                    for ($x = 1; $x <= $xMax; ++$x) {
                        ...
                        $colStart = ($x == 3) ? PHPExcel_Cell::stringFromColumnIndex(
                            $rangeEnd[0]
                        ) : PHPExcel_Cell::stringFromColumnIndex($rangeStart[0] + $x - 1);

                        // end column index for region
                        $colEnd = ($x == 1) ? PHPExcel_Cell::stringFromColumnIndex(
                            $rangeStart[0]
    ...
    public function getHashCode()
    {
        ...
        return md5(
            $this->_fill->getHashCode().$this->_font->getHashCode().$this->_borders->getHashCode(
            ).$this->_alignment->getHashCode().$this->_numberFormat->getHashCode(
