...
class PHPExcel_Writer_Excel5 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
    ...
    public function save($pFilename = null)
    {
                ...
                if ($cVal instanceof PHPExcel_RichText) {
                    ...
                    foreach ($elements as $element) {
                        if ($element instanceof PHPExcel_RichText_Run) {
                            ...
                            $this->_writerWorksheets[$i]->_fntHashIndex[$font->getHashCode(
                            )] = $this->_writerWorkbook->_addFont($font);
                        }
                    }
                }
            }
        ...
        if (isset($this->_documentSummaryInformation) && !empty($this->_documentSummaryInformation)) {
            $OLE_DocumentSummaryInformation = new PHPExcel_Shared_OLE_PPS_File(
                PHPExcel_Shared_OLE::Asc2Ucs(chr(5).'DocumentSummaryInformation')
            );
        ...
        if (isset($this->_summaryInformation) && !empty($this->_summaryInformation)) {
            $OLE_SummaryInformation = new PHPExcel_Shared_OLE_PPS_File(
                PHPExcel_Shared_OLE::Asc2Ucs(chr(5).'SummaryInformation')
            );
    ...
    private function _buildWorksheetEschers()
    {
        ...
        foreach ($this->_phpExcel->getAllsheets() as $sheet) {
            ...
            foreach ($sheet->getDrawingCollection() as $drawing) {
                ...
                $twoAnchor = PHPExcel_Shared_Excel5::oneAnchor2twoAnchor(
                    $sheet,
                    $coordinates,
                    $offsetX,
                    $offsetY,
            ...
            if (!empty($filterRange)) {
                ...
                while ($iInc <= $iNumColEnd) {
                    ...
                    // set coordinates and offsets, client anchor
                    $endCoordinates = PHPExcel_Cell::stringFromColumnIndex(
