...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter
{
    ...
    function close()
    {
        ...
        if (($defaultWidth = $_phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
            $defaultWidth = PHPExcel_Shared_Font::getDefaultColumnWidthByFont(
                $_phpSheet->getParent()->getDefaultStyle()->getFont()
            );
        }
    ...
    function _writeUrlExternal($row1, $col1, $row2, $col2, $url)
    {
        ...
        $data = pack(
                ...
                $col1,
                $col2
            ).$unknown1.$link_type.$unknown2.$up_count.$dir_short_len.$dir_short.$unknown3.$stream_len;
        /*.
								  $dir_long_len .
								  $unknown4	 .
								  $dir_long	 .
    ...
    private function _writeWindow2()
    {
        ...
        $fPaged = 1; // 2
        $fPageBreakPreview = $this->_phpSheet->getSheetView()->getView(
    ...
    private function _writeSheetProtection()
    {
        ...
        // prepare options
        $options = (int)!$this->_phpSheet->getProtection()->getObjects() | (int)!$this->_phpSheet->getProtection(
            )->getScenarios() << 1 | (int)!$this->_phpSheet->getProtection()
    ...
    private function _writeSetup()
    {
        ...
        $iScale = $this->_phpSheet->getPageSetup()->getScale() ? $this->_phpSheet->getPageSetup()->getScale(
        ...
        // Page orientation
        $fLandscape = ($this->_phpSheet->getPageSetup()->getOrientation(
            ) == PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE) ? 0x0 : 0x1;
    ...
    private function _writeHeader()
    {
		...
		*/
        ...
        $recordData = PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong(
            $this->_phpSheet->getHeaderFooter()->getOddHeader()
        );
        $length = strlen($recordData);
    ...
    private function _writeFooter()
    {
		...
		*/
        ...
        $recordData = PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong(
            $this->_phpSheet->getHeaderFooter()->getOddFooter()
        );
        $length = strlen($recordData);
    ...
    function _positionImage($col_start, $row_start, $x1, $y1, $width, $height)
    {
        ...
        if ($x1 >= PHPExcel_Shared_Excel5::sizeCol(
                $this->_phpSheet,
        ...
        while ($width >= PHPExcel_Shared_Excel5::sizeCol(
                $this->_phpSheet,
        ...
        $x1 = $x1 / PHPExcel_Shared_Excel5::sizeCol(
                $this->_phpSheet,
                PHPExcel_Cell::stringFromColumnIndex($col_start)
            ) * 1024;
        $y1 = $y1 / PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1) * 256;
        $x2 = $width / PHPExcel_Shared_Excel5::sizeCol(
                $this->_phpSheet,
                PHPExcel_Cell::stringFromColumnIndex($col_end)
            ) * 1024; // Distance to right side of object
        $y2 = $height / PHPExcel_Shared_Excel5::sizeRow(
                $this->_phpSheet,
                $row_end + 1
    ...
    private function _writeCFRule(PHPExcel_Style_Conditional $conditional)
    {
        ...
        if ($numConditions == 1) {
            ...
            if ($numConditions == 2 && ($conditional->getOperatorType(
        ...
        // Border
        $bBorderLeft = ($conditional->getStyle()->getBorders()->getLeft()->getColor()->getARGB(
        ) == PHPExcel_Style_Color::COLOR_BLACK && $conditional->getStyle()->getBorders()->getLeft()
            ->getBorderStyle() == PHPExcel_Style_Border::BORDER_NONE ? 1 : 0);
        $bBorderRight = ($conditional->getStyle()->getBorders()->getRight()->getColor()->getARGB(
        ) == PHPExcel_Style_Color::COLOR_BLACK && $conditional->getStyle()->getBorders()->getRight()
            ->getBorderStyle() == PHPExcel_Style_Border::BORDER_NONE ? 1 : 0);
        $bBorderTop = ($conditional->getStyle()->getBorders()->getTop()->getColor()->getARGB(
        ) == PHPExcel_Style_Color::COLOR_BLACK && $conditional->getStyle()->getBorders()->getTop()
            ->getBorderStyle() == PHPExcel_Style_Border::BORDER_NONE ? 1 : 0);
        $bBorderBottom = ($conditional->getStyle()->getBorders()->getBottom()->getColor()->getARGB(
        ) == PHPExcel_Style_Color::COLOR_BLACK && $conditional->getStyle()->getBorders()->getBottom()
            ->getBorderStyle() == PHPExcel_Style_Border::BORDER_NONE ? 1 : 0);
        ...
        // Font
        if ($conditional->getStyle()->getFont()->getName() != null || $conditional->getStyle()->getFont()->getSize(
            ) != null || $conditional->getStyle()->getFont()
        ...
        if ($bFormatFont == 1) {
            ...
            if ($conditional->getStyle()->getFont()->getName() == null) {
                ...
                $dataBlockFont = PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong(
                    $conditional->getStyle()->getFont()->getName()
                );
            }
            // Font Size
