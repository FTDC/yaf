...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
            ...
            foreach ($namedRanges as $namedRange) {
                ...
                try {
                    ...
                    $chunk .= $this->writeData(
                        $this->_writeDefinedNameBiff8($namedRange->getName(), $formulaData, $scope, false)
                    );
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isColumnsToRepeatAtLeftSet() && $sheetSetup->isRowsToRepeatAtTopSet()) {
                ...
                if ($sheetSetup->isColumnsToRepeatAtLeftSet() || $sheetSetup->isRowsToRepeatAtTopSet()) {
                    ...
                    // store the DEFINEDNAME record
                    $chunk .= $this->writeData(
                        $this->_writeDefinedNameBiff8(pack('C', 0x07), $formulaData, $i + 1, true)
                    );
                }
            }
        }
    ...
    private function _writeDatemode()
    {
        ...
        $f1904 = (PHPExcel_Shared_Date::getExcelCalendar(
            ) == PHPExcel_Shared_Date::CALENDAR_MAC_1904) ? 1 : 0; // Flag for 1904 date system
