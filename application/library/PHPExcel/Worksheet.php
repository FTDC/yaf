...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
                ...
                if (isset($autoSizes[$this->_cellCollection->getCurrentColumn()])) {
                    ...
                    if (!isset($isMergeCell[$this->_cellCollection->getCurrentAddress()])) {
                        ...
                        $cellValue = PHPExcel_Style_NumberFormat::toFormattedString(
                            ...
                            $this->getParent()->getCellXfByIndex($cell->getXfIndex())->getNumberFormat()
                                ->getFormatCode()
                        );
    ...
    public function setCellValueExplicitByColumnAndRow(
        ...
        $cell = $this->getCell(PHPExcel_Cell::stringFromColumnIndex($pColumn).$pRow)->setValueExplicit(
            $pValue,
            $pDataType
        );
    ...
    public function getCell($pCoordinate = 'A1')
    {
        ...
        if (strpos($pCoordinate, ':') !== false || strpos($pCoordinate, ',') !== false) {
            ...
            if (PHPExcel_Cell::columnIndexFromString(
                    $this->_cachedHighestColumn
    ...
    {
        $cellRange = PHPExcel_Cell::stringFromColumnIndex($pColumn1).$pRow1.':'.PHPExcel_Cell::stringFromColumnIndex(
                $pColumn2
            ).$pRow2;
    ...
    public function setAutoFilterByColumnAndRow($pColumn1 = 0, $pRow1 = 1, $pColumn2 = 0, $pRow2 = 1)
    {
        return $this->setAutoFilter(
            PHPExcel_Cell::stringFromColumnIndex($pColumn1).$pRow1.':'.PHPExcel_Cell::stringFromColumnIndex(
                $pColumn2
            ).$pRow2
        );
    }
                    ...
                    if ($cell->getValue() !== null) {
                        ...
                        if ($formatData) {
                            ...
                            $returnValue[$rRef][$cRef] = PHPExcel_Style_NumberFormat::toFormattedString(
                                ...
                                ($style->getNumberFormat()) ? $style->getNumberFormat()
    ...
    public function getHashCode()
    {
        if ($this->_dirty) {
            $this->_hash = md5(
                $this->_title.$this->_autoFilter.($this->_protection->isProtectionEnabled() ? 't' : 'f').__CLASS__
            );
            $this->_dirty = false;
        }
