...
class PHPExcel_Calculation
{
    ...
    public function setLocale($locale = 'en_us')
    {
        ...
        if (in_array($language, self::$_validLocaleLanguages)) {
            ...
            if ($locale != 'en_us') {
                ...
                $functionNamesFile = PHPEXCEL_ROOT.'PHPExcel'.DIRECTORY_SEPARATOR.'locale'.DIRECTORY_SEPARATOR.str_replace(
                        '_',
                        DIRECTORY_SEPARATOR,
                        $locale
                ...
                $configFile = PHPEXCEL_ROOT.'PHPExcel'.DIRECTORY_SEPARATOR.'locale'.DIRECTORY_SEPARATOR.str_replace(
                        '_',
                        DIRECTORY_SEPARATOR,
                        $locale
    ...
    public function calculateCellValue(PHPExcel_Cell $pCell = null, $resetLog = true)
    {
        ...
        try {
            $result = self::_unwrapResult(
                $this->_calculateFormulaValue($pCell->getValue(), $pCell->getCoordinate(), $pCell)
            );
        } catch (PHPExcel_Exception $e) {
            throw new PHPExcel_Calculation_Exception($e->getMessage());
        }
     ...
     */
    private static function _resizeMatricesShrink(
        &$matrix1,
        &$matrix2,
        $matrix1Rows,
        $matrix1Columns,
     ...
     * @param    integer $matrix2Columns Column size of second matrix operand
     */
    private static function _resizeMatricesExtend(
        &$matrix1,
        &$matrix2,
        $matrix1Rows,
        $matrix1Columns,
    ...
    private function _parseFormula($formula, PHPExcel_Cell $pCell = null)
    {
        ...
        while (true) {
            ...
            if ((isset(self::$_comparisonOperators[$opCharacter])) && (strlen(
                        $formula
                    ) > $index) && (isset(self::$_comparisonOperators[$formula{$index + 1}]))) {
            ...
            if ($opCharacter == '-' && !$expectingOperator) { //	Is it a negation instead of a minus?
                ...
                return $this->_raiseFormulaError(
                ...
                while ($stack->count() > 0 && ($o2 = $stack->last(
                    )) && isset(self::$_operators[$o2['value']]) && @(self::$_operatorAssociativity[$opCharacter] ? self::$_operatorPrecedence[$opCharacter] < self::$_operatorPrecedence[$o2['value']] : self::$_operatorPrecedence[$opCharacter] <= self::$_operatorPrecedence[$o2['value']])) {
                    $output[] = $stack->pop(
                ...
                if (preg_match(
                    ...
                    if ($argumentCountError) {
                        return $this->_raiseFormulaError(
                            "Formula Error: Wrong number of arguments for $functionName() function: $argumentCount given, ".$expectedArgumentCountString." expected"
                        );
                    }
                }
                ++$index;
                ...
                if (preg_match('/^'.self::CALCULATION_REGEXP_FUNCTION.'$/i', $val, $matches)) {
                    ...
                    if (isset(
                            self::$_PHPExcelFunctions[strtoupper(
                                $matches[1]
                            )]
                        ) || isset(self::$_controlFunctions[strtoupper($matches[1])])) { // it's a function
                    ...
                    if ($testPrevOp['value'] == ':') {
                        ...
                        if ((is_integer($startRowColRef)) && (ctype_digit(
                                $val
                            )) && ($startRowColRef <= 1048576) && ($val <= 1048576)) {
                            //	Row range
                            $endRowColRef = ($pCellParent !== null) ? $pCellParent->getHighestColumn(
                            ) : 'XFD'; //	Max 16,384 columns for Excel2007
                            $output[count($output) - 1]['value'] = $rangeWS1.'A'.$startRowColRef;
                            $val = $rangeWS2.$endRowColRef.$val;
                        } elseif ((ctype_alpha($startRowColRef)) && (ctype_alpha($val)) && (strlen(
                                    $startRowColRef
                                ) <= 3) && (strlen($val) <= 3)) {
                            //	Column range
                            $endRowColRef = ($pCellParent !== null) ? $pCellParent->getHighestRow(
                    ...
                    } elseif (($localeConstant = array_search(
                            trim(strtoupper($val)),
                            self::$_localeBoolean
                        )) !== false) {
            ...
            if ($formula{$index} == ' ') {
                ...
                if (($expectingOperator) && (preg_match(
            ...
            if (isset(self::$_binaryOperators[$token])) {
                ...
                if ($token == ':') {
                    $this->_debugLog->writeDebugLog(
                        'Evaluating Range ',
                        $this->_showValue($operand1Data['reference']),
                        ' ',
                        $token,
                        ' ',
                        $this->_showValue($operand2Data['reference'])
                    );
                } else {
                    $this->_debugLog->writeDebugLog(
                        'Evaluating ',
                        $this->_showValue($operand1),
                        ' ',
                        $token,
                        ' ',
                ...
                switch ($token) {
                    ...
                    case ':'    : //	Range
                        ...
                        if ($sheet1 == $sheet2) {
                            ...
                            $cellRef = PHPExcel_Cell::stringFromColumnIndex(min($oCol)).min(
                                    $oRow
                                ).':'.PHPExcel_Cell::stringFromColumnIndex(max($oCol)).max($oRow);
                            if ($pCellParent !== null) {
                                $cellValue = $this->extractCellRange(
                                    $cellRef,
                                    $this->_workbook->getSheetByName($sheet1),
                                    false
                                );
                            } else {
                        ...
                        break;
                    case '+'    : //	Addition
                        $this->_executeNumericBinaryOperation(
                            $cellID,
                            $operand1,
                            $operand2,
                            $token,
                    ...
                    case '&'    : //	Concatenation
                        ...
                        if ((is_array($operand1)) || (is_array($operand2))) {
                            ...
                            $result = '"'.str_replace(
                                    '""',
                                    '"',
                                    self::_unwrapResult($operand1, '"').self::_unwrapResult($operand2, '"')
                                ).'"';
                        }
                    ...
                    case '|'    : //	Intersect
                        ...
                        $this->_debugLog->writeDebugLog(
                            ...
                            $this->_showTypeDetails($cellIntersect)
                        );
                    ...
                    if ($pCell === null) {
                        ...
                        if ($matches[2] > '') {
                            ...
                            $this->_debugLog->writeDebugLog(
                                ...
                                $matches[2],
                                ' is ',
                                $this->_showTypeDetails($cellValue)
                            );
                ...
                if (isset(self::$_ExcelConstants[strtoupper($token)])) {
                    ...
                    $this->_debugLog->writeDebugLog(
                        ...
                        $excelConstant,
                        ' as ',
                        $this->_showTypeDetails(self::$_ExcelConstants[$excelConstant])
                    );
                } elseif ((is_numeric($token)) || ($token === null) || (is_bool(
                        $token
    ...
    private function _executeBinaryComparisonOperation(
        $cellID,
        $operand1,
        $operand2,
        $operation,
            ...
            if ((is_array($operand1)) && (!is_array($operand2))) {
                ...
                foreach ($operand1 as $x => $operandData) {
                    $this->_debugLog->writeDebugLog(
                        ...
                        $operation,
                        ' ',
                        $this->_showValue($operand2[$x])
                    );
                    $this->_executeBinaryComparisonOperation(
                        $cellID,
                        $operandData,
                        $operand2[$x],
                        $operation,
    ...
    private function _executeNumericBinaryOperation($cellID, $operand1, $operand2, $operation, $matrixFunction, &$stack)
    {
        ...
        if ($executeMatrixOperation) {
            ...
            if ((PHPExcel_Calculation_Functions::getCompatibilityMode(
                    ) != PHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE) && ((is_string(
                            $operand1
        ...
        if ($pSheet !== null) {
            ...
            if (!isset($aReferences[1])) {
                ...
                if ($pSheet->cellExists($aReferences[0])) {
                    $returnValue[$currentRow][$currentCol] = $pSheet->getCell($aReferences[0])->getCalculatedValue(
                        $resetLog
                    );
                } else {
                ...
                foreach ($aReferences as $reference) {
                    ...
                    if ($pSheet->cellExists($reference)) {
                        $returnValue[$currentRow][$currentCol] = $pSheet->getCell($reference)->getCalculatedValue(
                            $resetLog
                        );
                    } else {
                        $returnValue[$currentRow][$currentCol] = null;
        ...
        if ($pSheet !== null) {
            ...
            if ($namedRange !== null) {
                ...
                if (ctype_alpha($splitRange[0][0])) {
                    ...
                    $pRange = 'A'.$splitRange[0][0].':'.$namedRange->getWorksheet()->getHighestColumn(
                        ).$splitRange[0][1];
                }
