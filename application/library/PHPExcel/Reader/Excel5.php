...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    public function load($pFilename)
    {
        ...
        foreach ($this->_sheets as $sheet) {
            ...
            foreach ($this->_objs as $n => $obj) {
                ...
                if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
                    ...
                    list($startColumn, $startRow) = PHPExcel_Cell::coordinateFromString(
                        $spContainer->getStartCoordinates()
                    );
                    ...
                    $width = PHPExcel_Shared_Excel5::getDistanceX(
                        $this->_phpSheet,
                        $startColumn,
                        $startOffsetX,
                        $endColumn,
                    ...
                    $height = PHPExcel_Shared_Excel5::getDistanceY(
                        $this->_phpSheet,
                        $startRow,
                        $startOffsetY,
                        $endRow,
            ...
            if ($this->_version == self::XLS_BIFF8) {
                foreach ($this->_sharedFormulaParts as $cell => $baseCell) {
                    ...
                    if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell(
                            $column,
                            $row,
                            $this->_phpSheet->getTitle()
                        )) {
                        $formula = $this->_getFormulaFromStructure($this->_sharedFormulas[$baseCell], $cell);
                        $this->_phpSheet->getCell($cell)->setValueExplicit(
                            '='.$formula,
            ...
            if (!empty($this->_cellNotes)) {
                foreach ($this->_cellNotes as $note => $noteDetails) {
                    ...
                    $this->_phpSheet->getComment($cellAddress)->setAuthor($noteDetails['author'])->setText(
                        $this->_parseRichText($noteDetails['objTextData']['text'])
                    );
                }
            }
        }
        ...
        foreach ($this->_definedname as $definedName) {
            if ($definedName['isBuiltInName']) {
                ...
                if (count($explodes) == 2) {
                    if (($docSheet = $this->_phpExcel->getSheetByName(
                            $explodes[0]
                        ...
                        $scope = ($definedName['scope'] == 0) ? null : $this->_phpExcel->getSheetByName(
                            $this->_sheets[$definedName['scope'] - 1]['name']
                        );

                        $this->_phpExcel->addNamedRange(
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-time
                    $value = PHPExcel_Shared_OLE::OLE2LocalDate(
                        substr($this->_summaryInformation, $secOffset + 4 + $offset, 8)
                    );
                    break;
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x40: //	Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-Time
                    $value = PHPExcel_Shared_OLE::OLE2LocalDate(
                        substr($this->_documentSummaryInformation, $secOffset + 4 + $offset, 8)
                    );
                    break;
    ...
    private function _readXf()
    {
        ...
        if (!$this->_readDataOnly) {
            ...
            $objStyle->getProtection()->setLocked(
                $isLocked ? PHPExcel_Style_Protection::PROTECTION_INHERIT : PHPExcel_Style_Protection::PROTECTION_UNPROTECTED
            );

            // bit 1; mask 0x02; 1 = Formula is hidden
            $isHidden = (0x02 & $xfTypeProt) >> 1;
            $objStyle->getProtection()->setHidden(
                $isHidden ? PHPExcel_Style_Protection::PROTECTION_PROTECTED : PHPExcel_Style_Protection::PROTECTION_UNPROTECTED
            );
            ...
            if ($this->_version == self::XLS_BIFF8) {
                ...
                if ($bordersBottomStyle = self::_mapBorderStyle(
                    (0x0000F000 & self::_GetInt4d($recordData, 10)) >> 12
                )) {
                ...
                $objStyle->getBorders()->getDiagonal()->colorIndex = (0x001FC000 & self::_GetInt4d(
                            $recordData,
                            14
                        )) >> 14;

                // bit: 24-21; mask: 0x01E00000; diagonal style
                if ($bordersDiagonalStyle = self::_mapBorderStyle(
                    (0x01E00000 & self::_GetInt4d($recordData, 14)) >> 21
                )) {
                ...
                $objStyle->getBorders()->getBottom()->setBorderStyle(
                    self::_mapBorderStyle((0x01C00000 & $borderAndBackground) >> 22)
                );
                ...
                $objStyle->getBorders()->getTop()->setBorderStyle(
                    self::_mapBorderStyle((0x00000007 & $borderLines) >> 0)
                );

                // bit: 5-3; mask: 0x00000038; left line style
                $objStyle->getBorders()->getLeft()->setBorderStyle(
                    self::_mapBorderStyle((0x00000038 & $borderLines) >> 3)
                );

                // bit: 8-6; mask: 0x000001C0; right line style
                $objStyle->getBorders()->getRight()->setBorderStyle(
                    self::_mapBorderStyle((0x000001C0 & $borderLines) >> 6)
                );
                ...
                switch ($extType) {
                    ...
                    case 7: // border color top
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                $top = $this->_phpExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders(
                                )->getTop();
                    ...
                    case 8: // border color bottom
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                $bottom = $this->_phpExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders(
                                )->getBottom();
                    ...
                    case 9: // border color left
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                $left = $this->_phpExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders(
                                )->getLeft();
                    ...
                    case 10: // border color right
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                $right = $this->_phpExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])->getBorders(
                                )->getRight();
                    ...
                    case 11: // border color diagonal
                        ...
                        if ($xclfType == 2) {
                            ...
                            // modify the relevant style property
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                $diagonal = $this->_phpExcel->getCellXfByIndex(
                                    $this->_mapCellXfIndex[$ixfe]
                                )->getBorders()->getDiagonal();
    ...
    private function _readPageSetup()
    {
        ...
        if (!$this->_readDataOnly) {
            ...
            if (!$isNotInit) {
                ...
                switch ($isPortrait) {
                    case 0:
                        $this->_phpSheet->getPageSetup()->setOrientation(
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell(
            ...
            if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
                ...
                for ($i = 0; $i <= $sstCount; ++$i) {
                    if (isset($fmtRuns[$i])) {
                        $text = PHPExcel_Shared_String::Substring(
                            $this->_sst[$index]['value'],
                            $charPos,
                        ...
                        $text = PHPExcel_Shared_String::Substring(
                            $this->_sst[$index]['value'],
                            $charPos,
                            PHPExcel_Shared_String::CountCharacters($this->_sst[$index]['value'])
                        );
                    }
    ...
    private function _readMergedCells()
    {
        ...
        if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
            ...
            foreach ($cellRangeAddressList['cellRangeAddresses'] as $cellRangeAddress) {
                if ((strpos($cellRangeAddress, ':') !== false) && ($this->_includeCellRangeFiltered(
                        $cellRangeAddress
                    ))) {
    ...
    private static function _readUnicodeString($subData, $characterCount)
    {
        ...
        $value = self::_encodeUTF16(
            substr($subData, 1, $isCompressed ? $characterCount : 2 * $characterCount),
            $isCompressed
        );
