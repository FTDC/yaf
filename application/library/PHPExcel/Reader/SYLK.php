...
class PHPExcel_Reader_SYLK extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        while (($rowData = fgets($fileHandle)) !== false) {
            ...
            $rowData = explode(
                "\t",
                str_replace('¤', ';', str_replace(';', "\t", str_replace(';;', '¤', rtrim($rowData))))
            );
        ...
        while (($rowData = fgets($fileHandle)) !== false) {
            ...
            if ($dataType == 'P') {
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum{0}) {
                        case 'P' :
                            $formatArray['numberformat']['code'] = str_replace(
                                $fromFormats,
                                $toFormats,
                                substr($rowDatum, 1)
                            );
                            ...
                            foreach ($temp as &$value) {
                                ...
                                if ($key = !$key) {
                                    ...
                                    foreach ($cellReferences as $cellReference) {
                                        ...
                                        $A1CellReference = PHPExcel_Cell::stringFromColumnIndex(
                                                $columnReference - 1
                                            ).$rowReference;
