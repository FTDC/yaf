...
class PHPExcel_Calculation_DateTime
{
    ...
    private static function _dateDiff360($startDay, $startMonth, $startYear, $endDay, $endMonth, $endYear, $methodUS)
    {
        if ($startDay == 31) {
            --$startDay;
        } elseif ($methodUS && ($startMonth == 2 && ($startDay == 29 || ($startDay == 28 && !self::_isLeapYear(
                            $startYear
                        ))))) {
            $startDay = 30;
        }
    ...
    public static function _getDateValue($dateValue)
    {
        if (!is_numeric($dateValue)) {
            if ((is_string($dateValue)) && (PHPExcel_Calculation_Functions::getCompatibilityMode(
    ...
    public static function TIMEVALUE($timeValue)
    {
        ...
        if (($PHPDateArray !== false) && ($PHPDateArray['error_count'] == 0)) {
            ...
            switch (PHPExcel_Calculation_Functions::getReturnDateType()) {
                ...
                case PHPExcel_Calculation_Functions::RETURNDATE_PHP_OBJECT :
                    return new DateTime(
                        '1900-01-01 '.$PHPDateArray['hour'].':'.$PHPDateArray['minute'].':'.$PHPDateArray['second']
                    );
            }
        }
