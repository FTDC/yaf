...
abstract class Db
{
        ...
        foreach ($data as $row_data) {
            ...
            foreach ($row_data as $key => $value) {
                ...
                if ($value['type'] == 'sql') {
                    ...
                    $values[] = $null_values && ($value['value'] === '' || is_null(
                            $value['value']
                        )) ? 'NULL' : "'{$value['value']}'";
                }
            }
    ...
    public function update($table, $data, $where = '', $limit = 0, $null_values = false, $use_cache = true)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if ($value['type'] == 'sql') {
                ...
                $sql .= ($null_values && ($value['value'] === '' || is_null(
                            $value['value']
    ...
    public function nextId($table_name)
    {
        return $this->getValue(
            "select AUTO_INCREMENT from information_schema.TABLES where TABLE_SCHEMA='".$this->database."' and TABLE_NAME='".pSQL(
                $table_name
            )."'"
        );
    }
    ...
    public static function checkConnection($server, $user, $pwd, $db, $new_db_link = true, $engine = null, $timeout = 5)
    {
        return call_user_func_array(
            array(Db::getClass(), 'tryToConnect'),
            array($server, $user, $pwd, $db, $new_db_link, $engine, $timeout)
        );
    }
    ...
    {
        return call_user_func_array(
            array(Db::getClass(), 'hasTableWithSamePrefix'),
            array($server, $user, $pwd, $db, $prefix)
        );
    }
