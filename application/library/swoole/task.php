        ...
        if ($data['is_pay']) {
            if (isset($data['explodenum']) && !empty($data['explodenum'])) {
                for ($i = $data['prenum'] + 1; $i <= $numall; $i++) {
                    $result = array(
                        'pi' => $task->encrypt($data['goods_id'].'_'.$data['id'].'_'.$i, true),
                        'token' => md5($data['goods_id'].'_'.$data['id'].'_'.$i),
                        'num' => 0,
                    );
                    $url .= $task->encrypt($data['goods_id'].'_'.$data['id'].'_'.$i, true).'&token='.md5(
                            $data['goods_id'].'_'.$data['id'].'_'.$i
                        );
                    $task->twodemcode(
                        $data['goods_id'],
                        $data['id'],
