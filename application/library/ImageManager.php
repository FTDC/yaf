...
class ImageManager
{
    ...
    public static function checkImageMemoryLimit($image)
    {
        ...
        if (function_exists('memory_get_usage') && (int)$memory_limit != -1) {
            ...
            if (($infos[0] * $infos[1] * $infos['bits'] * $channel + pow(
                        2,
                        16
                    )) * 1.8 + $current_memory > $memory_limit - 1024 * 1024) {
                return false;
            }
        }
    ...
    public static function validateUpload($file, $max_file_size = 0)
    {
        ...
        if (!ImageManager::isRealImage($file['tmp_name'], $file['type']) || !ImageManager::isCorrectImageFileExt(
                $file['name']
            )) {
